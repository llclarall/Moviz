...
<div class="poster-details">
  ...
  <a href="sources.html?allocineId=${allocineId}#source-link-container" class="poster-link">Voir la source</a>
</div>
...
`; // nettoyer le SVG avant de redessiner mais garder les axes
  ...
  filteredFilms.forEach((film) => {
    ...
    for (let i = 0; i < Math.floor(barWidth / (stripeWidth + gap)); i++) {
      ...
      stripe.setAttribute("width", 0); // Initialement à 0
    ...
    // Ajouter la cassette
    const cassette = document.createElementNS(
      "http://www.w3.org/2000/svg",
      "rect"
    );
    ...
    const middleDetail = document.createElementNS(
      "http://www.w3.org/2000/svg",
      "rect"
    );
...
fetch("./filmsDataEnriched.json")
  ...
  .then((films) => {
    ...
    // Pour traduire les genres en français
...
// Bouton pour chaque genre sélectionné
selectedGenres.forEach((genre) => {
  const button = document.createElement("button");
  button.textContent = genreTranslations[genre];
  button.setAttribute("class", "filtre sono");
...
fetch("./filmsDataEnriched.json")
  ...
  // démarquer le filtre sélectionné
...
fetch("filmsDataEnriched.json")
  ...
  .then((films) => {
    ...
    films.forEach((film) => {
      const year = parseInt(film.releaseDate.split(" ").pop(), 10);
    ...
    const decadeData = Object.entries(decades).map(([label, films]) => ({
      label,
      value: films.length,
      films,
    }));
    ...
    const color = d3
      ...
      .range(["#f7b5", "#ffb265", "#fc7e54", "#b73131", "#b56f3f"]);
    ...
    function handleEvents(selection) {
      selection
        ...
        .on("click", function (event, d) {
          // Fonction pour obtenir 3 films aléatoires
          function getRandomFilms(films, count = 3) {
          ...
          function updateInfoBox() {
            ...
            const allocineId = new URLSearchParams(
            ...
            ${
              showingTopFilms
                ? `<span class="dont">dont les 3 meilleurs :</span>`
                : ""
            }
            <div class="films">
              ${displayedFilms
                .map(
                  (film) => `
                  <div class="film-item">
                    <a href="sources.html?allocineId=${
                      idAlloCine[film.imdb]
                    }#source-link-container" title="voir la source de l'affiche">
                      <img src="${
                    ...
                    <div class="film-info">
                      ...
                      <div class="film-meta">Box Office: <span>${
                        film.boxOffice ? "$" + film.boxOffice : "N/A"
                      }</span></div>
                      <div class="film-meta">IMDb: <span>${
                        film.IMDBratings || "N/A"
                      }</span></div>
                    </div>
                  </div>`
                )
